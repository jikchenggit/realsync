...
create or replace package body dsg.check_pkg as
  ...
  begin
    ...
    insert into dsg.check_tab
      (src_owner, src_table_name, tgt_owner, tgt_table_name , rule)
      select owner, table_name, 'JCK', table_name ,rule
        from dsg.rule_tables;
    commit;
      ...
      if v_ck_tab > 0 then
        begin
          select count(1)
           ...
           where flag = 0
             and (diff_count != 0 or diff_count is null);
          select src_owner, src_table_name, tgt_owner, tgt_table_name ,rule
            into v_src_owner, v_src_tab_name, v_tgt_owner, v_tgt_tab_name ,v_rule
            from dsg.check_tab
           where flag = 0
             and (diff_count != 0 or diff_count is null)
             and rownum <= 1
          ...
          v_tgt_sql := 'select /*+parallel(dc,8)+*/ count(1) from "' ||
                       v_tgt_owner || '"."' || v_tgt_tab_name || '"@dbverify dc';
